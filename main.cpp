/* 
   Simulation of the Most Reliable Path Routing (MRPR) for Underwater Sensor Networks
*/
...
{
	printf("WitLab: Simulation of Shortest-Path Routing (MRPR) for Underwater Sensor Networks\n");
...
{
	float transmissionProbability = -1;                                           // 米米米
	for (float d = 100; d <= 1000; d += 100) {
		determineSendingSuccessByModels(d, true, transmissionProbability);
	}
	fflush(tFile0);
}
...
void generateInitialDepartureEventsForSource()
{
	...
	for (int i = 0; i<numOfSourceNodes; i++) {
		...
		MRPRpacket newPacket = MRPRpacket(msg_type_DATA, 0, packetSN, i, i); 
...
bool generateNewDepartureEventsForSource(int iNode)
{
	...
	// packet format: message type, hop count, packet serial number, sender node ID, source node ID
	MRPRpacket newPacket = MRPRpacket(msg_type_DATA, 0, packetSN, iNode, iNode);
...
void dataPacketProcessing(int iEvent)
{
	...
	if(remainingEnergy >= (powerReceiving - powerIdling)*timeslotLength) {
		...
		// 若此event不是到達sink的arrival event，則執行MRPR的forwarding algorithm
		if(!checkPacketArriveSink(iEvent)) {          
			MRPR_PacketForwardingAlgorithm(iEvent);
		}
	}
	else {
...
void MRPR_PacketForwardingAlgorithm(int eIndex)
{
	...
	if (!bDropPacket) {  // 若通過檢查(非drop)
		...
		// packet format: message type, hop count, packet serial number, sender node ID, source node ID
		MRPRpacket newPacket = MRPRpacket(msg_type_DATA, eventQueue[eIndex].packet.hop_count, 
			                            eventQueue[eIndex].packet.packetSN, eventQueue[eIndex].nodeID, 
						     			eventQueue[eIndex].packet.sourceNodeID);

		int HT = rand()%10 + 1;        // holding time為random 1~10個time slots 
...
void departurePacketProcessing(int iEvent)
{
	if (eventQueue[iEvent].packet.packetSN == 1)      // 觀察packet #1的走向 (just for debugging)
		fprintf(tFile1, "packet %d (from node %d) departured at node %d when %d \n", eventQueue[iEvent].packet.packetSN, 
		        eventQueue[iEvent].packet.senderNodeID, eventQueue[iEvent].nodeID, eventQueue[iEvent].occurringTime);
...
void generateArrivalEventsForNeighbors(int eIndex)
{
	...
	for (int n = 0; n<iNeighborSize; n++) {
		...
		float transmissionProbability = -1;                                                                        // 米米米
bool sendingSuccess = determineSendingSuccessByModels(neighbors[n].nodeDistance, false, transmissionProbability);  // 米米米
		if (sendingSuccess) {
			...
			MRPRpacket newPacket;
			if(eventQueue[eIndex].packet.messageType == msg_type_DATA) {
				// packet format: message type, hop count, packet serial number, sender node ID, source node ID
				newPacket = MRPRpacket(msg_type_DATA, eventQueue[eIndex].packet.hop_count, eventQueue[eIndex].packet.packetSN, 
...
vector<NeighborNode> scanNeighbors(int nodeID, int txRange)
{
    ...
    MRPR tempNode = sensorNode[nodeID];
...
//----------------------------------------------------
bool determineSendingSuccessByModels(float nodeDistance, bool printValue, float &transProb)
{
	...
	transProb = successProbability;                                        // 米米米
...
void dijkstraPreprocessing()
{
	...
	Dijkstra_Algorithm();          // 先後建出 1) shortestPathTreeAll 和 2) shortestPathTreeSink (註:shortestPathTreeSink才是MRPR用的tree)
	...
	for (int i = 0; i<numOfNodes; i++) {
		for (int j = 0; j<numOfNodes; j++) {
			if (sensorNetworkGraph[i][j] == 0.0) {
				...
				if (distance <= transmissionRangeMax) { // 距離小於transmissionRangeMax才有edge(E)

					float transmissionProbability = -1;                                           // 米米米
					determineSendingSuccessByModels(distance, false, transmissionProbability);    // 米米米
					float reliableDistance = -log10(transmissionProbability);                     // 米米米

					sensorNetworkGraph[i][j] = reliableDistance;                     // 米米米
					sensorNetworkGraph[j][i] = reliableDistance;                     // 米米米
				}
			}
		}
	}
...
void Dijkstra_Algorithm()
{
	for (int i = 0; i<numOfSourceNodes; i++) {
		for (int j = 0; j<numOfNodes; j++) {
			...
			// 註: 實際 MRPR 是使用 shortestPathTreeSink 做 forwarding 唷 (很重要!!)
		}
	}
	for (int i = 0; i<numOfSourceNodes; i++) {
...
bool Dijkstra_InsertPathSink(int src, int &numOfPaths)
{
	...
	sensorNode[src].bIsTreePathNode = true;         // 標記src在MRPR的tree上
	numOfPaths = 0;
