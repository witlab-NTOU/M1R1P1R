...
#define traceFileName_0         "MRPR_0_channel_model.log"             // channel model (transmitter power, node distance, bit error rate)
#define traceFileName_1         "MRPR_1_neighborhood.log"              // neighbors of each node
#define traceFileName_2         "MRPR_2_events_and_collisions.log"     // number of events and number of collisions
#define traceFileName_3         "MRPR_3_tree_table_for_all_nodes.log"  // shortest path tree table for "all nodes"
#define traceFileName_4         "MRPR_4_max_number_collisions.log"     // max number of collisions of one slot
#define traceFileName_5         "MRPR_5_info_departure_event.log"      // information of departure events
#define traceFileName_6         "MRPR_6_info_arrival_event.log"        // information of arrival events
#define traceFileName_7         "MRPR_7_tree_table_for_all_sinks.log"  // shortest path tree table for "all sinks"
#define traceFileName_8         "MRPR_8_hop_count.log"                 // hop count of each packet
#define traceFileName_9         "MRPR_9_energy_consumption.log"        // energy consumption of each node
#define traceFileName_10        "MRPR_10_node_position.log"            // position of source, sink, relay nodes
#define traceFileName_11        "MRPR_11_node_position_mobile.log"     // position of nodes (different time slots)
#define traceFileName_12        "MRPR_12.log"                          // 保留(需要時 可自行定義)
...
typedef struct MRPR_Packet  // Packet format
{
	int messageType;                        // "0"為 data 之後可定義不同 type (保留)
    int senderNodeID, sourceNodeID;         // MRPR 的 header 記錄 sender 和 source 
	int packetSN;
    int hop_count;                          // number of hops (from source to sink)
    //char packetData[packetDataLength];    // 佔記憶體空間故不模擬

    MRPR_Packet() {}
    MRPR_Packet(int mt, int hc, int psn, int sdid, int srcid) {         // DATA packet
        messageType = mt;
		hop_count = hc;
		packetSN = psn;
		senderNodeID = sdid;
...
typedef struct MRPR_Node  // Data structure for node
{
    int nX, nY, nZ;
    int nodeType;
    ...
    MRPR_Node() {}
    MRPR_Node(int x, int y, int z, int t) {
        nX = x;
        nY = y;
        nZ = z;
        nodeType = t;  
...
typedef struct Event_Item  // Data structure for event item
{
    ...
    */
    ...
    Event_Item() {}
    Event_Item(int t, int o, int i, int ss, MRPRpacket p) {
        eventType = t;
        occurringTime = o;
        nodeID = i;
		sourceNodeID = ss;
...
bool determineSendingSuccessByModels(float nodeDistance, bool printValue, float &transProb); // 米米米
